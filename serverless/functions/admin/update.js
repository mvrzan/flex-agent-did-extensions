...
const ServerlessOperations = require(Runtime.getFunctions()[
  'common/twilio-wrappers/serverless'
].path);

exports.handler = TokenValidator(async function update(
  context,
  event,
  callback
) {
  ...
  try {
    // get latest build
    const latestBuildResult = await ServerlessOperations.fetchLatestBuild({
      scriptName,
      context,
      attempts: 0,
    });
    ...
    const assetVersion = latestBuild.assetVersions.find(
    ...
    const functionVersions = latestBuild.functionVersions.map(
      version => version.sid
    );
